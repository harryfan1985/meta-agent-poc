...
_PLAN_SYS = (
    ...
    "Keep regex_match patterns LOOSE — match only an essential token (e.g. the function name "
    "via `def\\s+name`); never pin exact parameter names, spacing, type hints or full signatures "
    "(that false-rejects valid variants). Prefer field_present when in doubt. "
