...
**M0/M1 + opencode 对接 + 可观测 + M2(thin pipeline + task-level eval)已落地,M3 verifier 后端栈 + 校准协议起步**(`src/meta_agent/`,186 测试,覆盖约 94%,确定性测试不依赖真实 LLM):
...
| **M3 校准** | `calibration`:对 verifier 后端跑 golden cases,量化 precision/recall/F1/false-accept/false-reject;`calibrated_backends` 按阈值过滤(未达标不进 hot path)。bitfun 实测 BaseJudge/AspectPanel 在 5 条 golden 上 F1=1.0、false-accept=0(集偏清晰,borderline 集待补) | 🟡 |
| **M3 其余** | claim-evidence 残差 / mutation testing / benchmark 级评测 + 消融 / 沙箱加固 / Stage 3 真实 grounding | ⏭️ |
