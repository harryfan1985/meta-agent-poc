...
**M0–M3 框架已落地**(verifier 后端栈 / 校准 / claim-evidence / mutation / 沙箱加固 / 消融 / HumanEval benchmark + 正确性 oracle / flaky 确定性根治),`src/meta_agent/`,239 测试,覆盖约 94%,确定性测试不依赖真实 LLM:
...
| **M3 benchmark** | `code_sandbox`(子进程 + setrlimit + wall 超时跑不可信代码)+ `benchmarks`(HumanEval 加载 + 确定性正确性 oracle)+ `eval_harness`/`ablation` 接 oracle 计分;`run_benchmark` 多次跑给 per-problem reliable/flaky/never 统计。bitfun 实测(builtin 12 题×3=36):**pass@1 = 31/36 ≈ 86%**,**0 个 oracle/incorrect**——流水线从没写错代码,残余失败全是 execute 期数据流错配(非 codegen 质量)。经多处修复达此(无围栏 codegen + preflight 数据流检查 + 单 agent 框定 + 宽松 regex 降 false-reject + 执行期上游重跑恢复) | 🟡 |
| **M3 可靠性** | flaky 根因定位(4 次实测:残余是 ~15-20% 每次随机的 LLM stochasticity,两源=planner 过度分解 + 入口字段命名)+ 确定性根治:`Budget.max_specs`(原子任务设 1,拒多节点)与 `available_inputs` preflight(拒入口要 task_input 没有的字段),两源都在构造期拦下→重规划重采样。实测原 5 道 flaky 在 max_specs=1 下 **10/10 全 reliable** | 🟡 |
| **M3 ⏭️** | 真实 HumanEval 全集 + 更多数据集(MBPP/GSM8K)/ verifier 防偏差 swap-test / 真实 web_search provider / 成本预算评测 | ⏭️ |
