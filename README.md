...
**M0–M3 框架已落地**(verifier 后端栈 / 校准 / claim-evidence / mutation / 沙箱加固 / 消融),`src/meta_agent/`,217 测试,覆盖约 94%,确定性测试不依赖真实 LLM:
...
| **M3 benchmark** | benchmark 级 task-level 评测(6 数据集 + 每任务正确性 oracle,复现论文 −7.1/−5.5 消融分);verifier 防偏差 swap-test;真实 web_search provider | ⏭️ |
