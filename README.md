...
| **M3 benchmark** | `code_sandbox`(子进程 + setrlimit + wall 超时跑不可信代码)+ `benchmarks`(HumanEval 加载 + 确定性正确性 oracle)+ `eval_harness`/`ablation` 接 oracle 计分。bitfun 实测(builtin 12 题 × 3 次 = 36 attempts,`run_benchmark` 多次跑统计):**pass@1 = 31/36 ≈ 86%**;7 题 reliable(每次都过)、5 题 flaky(2/3)、**0 题 never**。关键:**全部失败都在 execute 期(ContractMismatch),0 个 oracle/incorrect**——即 36 次里流水线从没写错代码,残余 14% 纯属流水线鲁棒性(偶发过度分解→执行期数据流错配),非 codegen 质量。难题边界(触底=0、大小写折叠、无尾随分隔符等)模型均处理正确。经多处修复达此水平(无围栏 codegen + preflight 数据流检查 + 原子任务单 agent 框定 + 宽松 regex 降 false-reject + 执行期上游重跑恢复)。**flaky 根因定位 + 确定性根治**:4 次实测证实残余 flakiness 是 ~15-20% 每次随机的 LLM stochasticity,两源——planner 过度分解 + 入口节点字段命名。两个确定性杠杆把两源都在构造期拦下→重规划重采样:`Budget.max_specs`(原子任务设 1,拒多节点)+ `available_inputs` preflight(拒入口节点要 task_input 没有的字段)。实测:原 5 道 flaky 题在 max_specs=1 下 **10/10 全 reliable**(0 flaky)。oracle 计分消融发现验证在简单任务上的 false-reject(delta −0.33 → 0) | 🟡 |
